...
func NewSuperBlock(file string, c *conf.Config) (b *SuperBlock, err error) {
	...
	if err = b.init(); err != nil {
		log.Logger.Errorf("block: %s init() error(%v)", file, err)
		b.Close()
		return nil, err
	}
	return
...
func (b *SuperBlock) init() (err error) {
	...
	if stat, err = b.r.Stat(); err != nil {
		log.Logger.Errorf("block: %s Stat() error(%v)", b.File, err)
		return
	}
	if b.Size = stat.Size(); b.Size == 0 {
		...
		if err = b.parseMeta(); err != nil {
			log.Logger.Errorf("block: %s parseMeta() error(%v)", b.File, err)
			return
		}
		if _, err = b.w.Seek(_headerOffset, os.SEEK_SET); err != nil {
			log.Logger.Errorf("block: %s Seek() error(%v)", b.File, err)
			return
		}
	}
...
func (b *SuperBlock) flush(force bool) (err error) {
	...
	if b.conf.Block.Syncfilerange {
		if err = myos.Syncfilerange(fd, offset, size, myos.SYNC_FILE_RANGE_WRITE); err != nil {
			log.Logger.Errorf("block: %s Syncfilerange() error(%v)", b.File, err)
			b.LastErr = err
			return
		}
	} else {
		if err = myos.Fdatasync(fd); err != nil {
			log.Logger.Errorf("block: %s Fdatasync() error(%v)", b.File, err)
			b.LastErr = err
			return
		}
	}
	if err = myos.Fadvise(fd, offset, size, myos.POSIX_FADV_DONTNEED); err == nil {
		...
		log.Logger.Errorf("block: %s Fadvise() error(%v)", b.File, err)
		b.LastErr = err
	}
	return
}
...
// Delete log.Loggerical del a needls, only update the flag to it.
...
func (b *SuperBlock) Scan(r *os.File, offset uint32, fn func(*needle.Needle, uint32, uint32) error) (err error) {
	...
	if fi, err = r.Stat(); err != nil {
		log.Logger.Errorf("block: %s Stat() error(%v)", b.File)
		return
	}
	if err = myos.Fadvise(fd, bso, fi.Size(), myos.POSIX_FADV_SEQUENTIAL); err != nil {
		log.Logger.Errorf("block: %s Fadvise() error(%v)", b.File)
		return
	}
	log.Logger.Infof("scan block: %s from offset: %d", b.File, offset)
	if _, err = r.Seek(bso, os.SEEK_SET); err != nil {
		log.Logger.Errorf("block: %s Seek() error(%v)", b.File)
		return
	}
	for {
		if err = n.ParseFrom(rd); err != nil {
			if err != io.EOF {
				log.Logger.Errorf("block: parse needle from offset: %d:%d error(%v)", so, eo, err)
			}
			break
		}
		if n.TotalSize > int32(b.conf.BlockMaxSize) {
			log.Logger.Errorf("scan block: %s error(%v)", n, errors.ErrNeedleSize)
			err = errors.ErrNeedleSize
			break
		}
		log.Logger.Info(n.String())
		eo += n.IncrOffset
		if err = fn(n, so, eo); err != nil {
			log.Logger.Errorf("block: callback from offset: %d:%d error(%v)", so, eo, err)
			break
		}
		so = eo
	}
	if err == io.EOF {
		...
		log.Logger.Infof("scan block: %s to offset: %d [ok]", b.File, eo)
		err = nil
	} else {
		log.Logger.Infof("scan block: %s to offset: %d error(%v) [failed]", b.File, eo, err)
	}
	return
}
...
func (b *SuperBlock) Recovery(offset uint32, fn func(*needle.Needle, uint32, uint32) error) (err error) {
	...
	if b.Size != rsize {
		log.Logger.Warnf("block: %s [real size: %d, offset: %d] but [size: %d, offset: %d] not consistency, truncate file for force recovery, this may lost data",
			b.File, b.Size, needle.NeedleOffset(b.Size),
			rsize, b.Offset)
		// truncate file
		if err = b.w.Truncate(rsize); err != nil {
			log.Logger.Errorf("block: %s Truncate() error(%v)", b.File, err)
		}
	}
	return
}
...
func (b *SuperBlock) Compact(offset uint32, fn func(*needle.Needle, uint32, uint32) error) (err error) {
	...
	if r, err = os.OpenFile(b.File, os.O_RDONLY, 0664); err != nil {
		log.Logger.Errorf("os.OpenFile(\"%s\") error(%v)", b.File, err)
		return
	}
	...
	if err = r.Close(); err != nil {
		log.Logger.Errorf("block: %s Close() error(%v)", b.File, err)
	}
	return
}
...
func (b *SuperBlock) Close() {
	...
	if b.w != nil {
		if err = b.flush(true); err != nil {
			log.Logger.Errorf("block: %s flush error(%v)", b.File, err)
		}
		if err = b.w.Sync(); err != nil {
			log.Logger.Errorf("block: %s sync error(%v)", b.File, err)
		}
